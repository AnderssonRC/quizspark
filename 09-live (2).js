...
function StudentLive({ sessionId, participantId, quizInitial, onExit }) {
  ...
  // Al revelar (showResults), usar el resultado local guardado al responder.
  // Si no respondió esta pregunta, marcar como "sin respuesta".
  useEffectL(() => {
    if (session?.status === "showResults" && session.currentQuestionIdx >= 0) {
      if (answeredAtIdx === session.currentQuestionIdx && myResultThisQ) {
        setLastResult(myResultThisQ);
      } else {
        setLastResult({ noAnswer: true });
      }
    } else {
      setLastResult(null);
    }
  }, [session?.status, session?.currentQuestionIdx, myResultThisQ, answeredAtIdx]);
